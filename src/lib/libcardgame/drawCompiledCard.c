...
void _drawCompiledCard(byte cardValue, byte cardSuit, byte byteColumn, byte pixelRow)
{
    ...
    // Draw top of card.
    _drawCompiledPixMap(byteColumn,     pixelRow, cardTopLeft,  ROWS_PER_CARD_TOP, 1);
    _drawCompiledPixMap(byteColumn + 2, pixelRow, cardTopRight, ROWS_PER_CARD_TOP, 1);
    _drawCompiledPixMap(byteColumn + 2, pixelRow + ROWS_PER_CARD_TOP, cardRight, 1,
                        ROWS_PER_CARD_TOP * 2);
    ...
    _drawCompiledPixMap(byteColumn, pixelRow + ROWS_PER_CARD_TOP, pixmap, ROWS_PER_CARD_VALUE, 1);
    ...
    _drawCompiledPixMap(byteColumn,     middleRow, cardLeft,  1, middleHeight);
    _drawCompiledPixMap(byteColumn + 2, middleRow, cardRight, 1, middleHeight);

    // Draw bottom of card.
    _drawCompiledPixMap(byteColumn,     middleRow + middleHeight, cardBottomLeft,  ROWS_PER_CARD_BOTTOM,
                        1);
    _drawCompiledPixMap(byteColumn + 2, middleRow + middleHeight, cardBottomRight, ROWS_PER_CARD_BOTTOM,
                        1);
    ...
    if (cardSuit == SUIT_JOKER) {
        _drawCompiledPixMap(suitByteOffset, suitRow, cardValue == 1 ? redJokerSuit : blackJokerSuit,
                            ROWS_PER_SUIT, 2);
    } else {
        _drawCompiledPixMap(suitByteOffset, suitRow, suits[cardSuit], ROWS_PER_SUIT, 2);
    }
}
